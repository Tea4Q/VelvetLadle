...
- **Added `docs/REVENUECAT_SETUP.md`**: Step-by-step guide for configuring iOS (App Store Connect) and Android (Google Play Console) subscription products, linking them to RevenueCat entitlements/offerings, sandbox testing, and a pre-launch checklist for both platforms
- **Updated `docs/GOOGLE_PLAY_RELEASE.md`**: Added in-app purchase setup steps, RevenueCat Android key configuration, EAS secret instructions, subscription sandbox testing checklist, and updated version/next-steps section to v2.3.0
- **Updated `docs/FEATURES.md`**: Premium subscription section now references RevenueCat, lists monthly/annual options, and links to `REVENUECAT_SETUP.md`
...
_For detailed technical documentation, see [RECIPE_MANAGEMENT_ENHANCEMENTS.md](RECIPE_MANAGEMENT_ENHANCEMENTS.md)_

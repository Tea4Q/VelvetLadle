          ...
          onPress: async () => {
            try {
              if (favorite.type === "recipe" && favorite.recipe_id) {
                await FavoritesService.removeRecipeFromFavorites(
                  favorite.recipe_id,
                );
    ...
    try {
      const url =
        favorite.type === "recipe"
          ? favoriteRecipes.find((r) => r.id === favorite.recipe_id)
              ?.web_address
          : favorite.url;
    ...
    >
      <View style={styles.favoriteHeader}>
          ...
          <View style={styles.titleRow}>
            ...
            <View
              style={[
                ...
                {
                  backgroundColor:
                    favorite.type === "recipe" ? colors.primary : colors.accent,
        ...
        <View style={styles.favoriteActions}>
          ...
          >
            <FontAwesomeIcon name="trash" size={16} color={colors.error} />
          </TouchableOpacity>
        </View>
      </View>
    </TouchableOpacity>
      ...
      {displayData.length === 0 ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            `${item.type}-${item.id || item.url}-${index}`
          }
